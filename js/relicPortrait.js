...
/** Offscreen portraits for sealed / opened relics. */
...
/**
 * @param {string} relicId — defId or 'blackPackage'
 * @returns {string} data URL
 */
export function getRelicPortrait(relicId) {
  const id = relicId || 'blackPackage';
  const key = `${id}@v1`;
  ...
  const mesh = createRelicMesh(id, gradientMap);
...
export function clearRelicPortraitCache() {
  cache.clear();
}

/** HTML face for a warehouse / shop relic item */
export function relicFaceHtml(item) {
  const id = !item || item.sealed ? 'blackPackage' : (item.defId || 'blackPackage');
  return `<img src="${getRelicPortrait(id)}" alt="" draggable="false" />`;
}
