...
import { createCrystalGeometry, createAsteroidGeometry, createBoltRibbonGeometry, hash11 } from './geometry.js?v=29u';
...
function iceMaterial() {
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      uTime: { value: 0 },
      uColorDeep: { value: new THREE.Color(0x1a4a88) },
      uColorIce: { value: new THREE.Color(0xc8eeff) },
      uColorRim: { value: new THREE.Color(0xf4ffff) },
      uColorCore: { value: new THREE.Color(0x7ad8ff) },
    },
    vertexShader: /* glsl */ `
    ...
    fragmentShader: /* glsl */ `
      ...
      ${NOISE}
      void main() {
        vec3 N = normalize(cross(dFdx(vIceWorld), dFdy(vIceWorld)));
        ...
        body = mix(body, uColorRim, veins * 0.25);
        body = mix(body, uColorRim, cracks * 0.25);
        ...
        vec3 glow = uColorRim * pow(1.0 - ndv, 2.4) * 2.0;
        glow += uColorCore * (cracks * 0.5 + veins * 0.16) * 1.5;
        glow += uColorRim * sp * 2.4;
        glow += uColorCore * vIceBirth * 3.2;
        glow /= 1.0 + glow * 0.22;
        float alpha = clamp(0.62 + 0.5 * fres + cracks * 0.12, 0.0, 1.0);
        gl_FragColor = vec4(body + glow, alpha);
      }
    `,
  });
}
...
function meteorMaterial() {
  return new THREE.ShaderMaterial({
    ...
    vertexShader: /* glsl */ `
      ...
      void main(){
        vRockLocal=position;
        vRockNormalW=normalize(mat3(modelMatrix)*normal);
    ...
    fragmentShader: /* glsl */ `
      ...
      ${NOISE}
      void main(){
        vec3 N=normalize(cross(dFdx(vRockLocal), dFdy(vRockLocal)));
        float ndv=clamp(dot(normalize(vViewPosition), normalize(vViewPosition)),0.0,1.0);
        ndv=clamp(abs(dot(N, normalize(vViewPosition))),0.0,1.0);
        float rim=pow(1.0-ndv,2.2);
        vec3 p=vRockLocal*2.4;
        float f1=fbm3(p); float f2=fbm3(p*2.7+11.3);
        ...
        vec3 rock=mix(vec3(0.12,0.08,0.07), vec3(0.04,0.03,0.03), smoothstep(0.3,0.85,fbm3(vRockLocal*3.4)*0.5+0.5));
        rock=mix(rock, vec3(0.04,0.03,0.03), lip*0.8);
        rock*=1.0-fissure*0.92;
        rock*=mix(0.55,1.15,ndv);
        float heat=fissure*flow;
        vec3 glow=mix(vec3(1.0,0.28,0.05), vec3(1.0,0.85,0.35), core*core)*heat*3.2;
        float charge2=uCharge*uCharge;
        glow+=vec3(1.0,0.28,0.05)*rim*1.1*charge2;
        float lead=pow(clamp(dot(normalize(vRockNormalW), uHeading),0.0,1.0),2.6);
        glow+=vec3(1.0,0.85,0.35)*lead*1.6*charge2;
        glow/=1.0+glow*0.22;
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main(){
        ...
        float alpha = edge * (0.35 + frost*0.45 + plates*0.4) * uFade;
        if(alpha < 0.02) discard;
        vec3 col = mix(vec3(0.55,0.85,1.0), vec3(0.92,0.98,1.0), plates);
...
function zoneMaterial() {
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main(){
        vec2 p=(vUv-0.5)*2.0;
        float r=length(p)*uRadius*1.35;
        float ring=smoothstep(0.35,0.0,abs(r-uRadius));
        float fill=smoothstep(uRadius, uRadius*0.15, r)*uFill;
...
function spawnIce(root, origin, dir, range, fx) {
  ...
  for (let i = 0; i < ICE_COUNT; i++) {
    ...
    recs.push({
      along,
      lateral: (hash11(i * 3.17) - 0.5) * (0.5 + along * 2.4),
      h: 1.1 + along * 3.4 + hash11(i * 9.1) * 1.1,
      r: 0.7 + along * 1.15 + hash11(i * 5.3) * 0.4,
  ...
  function update(dt) {
    ...
    while (front - frostMark >= frostStep) {
      ...
      fx.sparks.emit(3, {
        ...
        speed: 1.3, spread: 1, life: 0.85, size: 18, color: 0xb8e8ff, gravity: 0.35, vy: 1,
      });
      fx.sparks.emit(5, {
        x: px, y: Y_WATER + 0.55, z: pz,
        speed: 2.1, spread: 0.85, life: 0.65, size: 5.5, color: 0xffffff, gravity: 0.85, vy: 1,
      });
    }
    ...
    if (mistN > 0) {
      fx.sparks.emit(mistN, {
        ...
        radius: 0.7, speed: 1.2, spread: 1, life: 0.8, size: 16, color: 0xc8eeff, gravity: 0.25, vy: 1,
      });
    }
    glitterAcc += dt * 22;
    ...
    if (glN > 0) {
      fx.sparks.emit(glN, {
        ...
        radius: 0.85, speed: 1.8, spread: 0.9, life: 0.55, size: 5, color: 0xf4ffff, gravity: 0.9, vy: 1,
      });
    }
    ...
    if (uFront >= 0.98) {
      ...
      if (!impacted) {
        ...
        fx.sparks.emit(26, {
          ...
          speed: 4.2, spread: 1, life: 1.05, size: 16, color: 0xc8eeff, gravity: 0.3, vy: 1,
        });
        fx.sparks.emit(36, {
          x: ix, y: Y_WATER + 0.6, z: iz, radius: 0.7,
          speed: 5.0, spread: 0.9, life: 0.8, size: 6, color: 0xffffff, gravity: 0.85, vy: 1,
        });
      }
    }
    light.intensity = 6 * (1 - age / life);
    for (let i = 0; i < ICE_COUNT; i++) {
      const rec = recs[i];
...
function spawnThunder(root, origin, dir, range, fx) {
  const group = new THREE.Group();
  root.add(group);
  const ox = origin.x;
  const oz = origin.z;
  ...
  function update(dt) {
    ...
    if (sn > 0 && vis > 0.05) {
      ...
      fx.sparks.emit(sn, {
        x: px, y: py, z: pz, radius: 0.35,
        speed: 3.4, spread: 0.95, life: 0.28, size: 5.5, color: 0xd8f0ff, gravity: 0.15,
    ...
    if (progress >= 1) {
      ...
      if (!impacted) {
        ...
        fx.sparks.emit(32, {
          x: tx, y: Y_WATER + 0.55, z: tz, radius: 0.5,
          speed: 6.5, spread: 1, life: 0.45, size: 6.5, color: 0xe8f4ff, gravity: 0.2, vy: 1,
        });
        fx.sparks.emit(14, {
          x: tx, y: Y_WATER + 0.4, z: tz, radius: 0.7,
          speed: 2.2, spread: 0.9, life: 0.9, size: 14, color: 0x88c8ff, gravity: 0.15, vy: 1,
        });
      }
    }
    if (age >= life) {
...
function spawnMeteor(root, origin, dir, range, fx) {
  const group = new THREE.Group();
  root.add(group);
  ...
  ring.material.uniforms.uRadius.value = 8;
  ring.visible = false;
  ring.renderOrder = 9;
  group.add(ring);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    sm.opacity = 0.72 * (1 - i * 0.16);
    const s = new THREE.Sprite(sm);
    s.scale.set(1.6 + i * 0.45, 1.6 + i * 0.45, 1);
    s.renderOrder = 12;
    group.add(s);
    trails.push(s);
  }
  ...
  function update(dt) {
    age += dt;
    mat.uniforms.uTime.value = age;
    mat.uniforms.uHeading.value.copy(heading);
    if (age < fly) {
      const u = age / fly;
      const ease = u * u;
      rock.position.set(
      ...
      rock.scale.setScalar(1.1 + u * 0.35);
      mat.uniforms.uCharge.value = u;
      light.position.copy(rock.position);
      light.intensity = 4 + u * 7;
      ...
      if (en > 0) {
        fx.sparks.emit(en, {
          ...
          speed: 3.2, spread: 0.85, life: 0.55, size: 8, color: 0xff8a28, gravity: 0.4,
        });
      }
      sparkAcc += dt * 28;
      ...
      if (sp > 0) {
        fx.sparks.emit(sp, {
          x: rock.position.x, y: rock.position.y, z: rock.position.z, radius: 0.25,
          vx: -heading.x, vy: -heading.y, vz: -heading.z,
          speed: 5.5, spread: 0.95, life: 0.32, size: 5, color: 0xffe090, gravity: 0.2,
        });
      }
    } else {
      if (!impacted) {
        ...
        fx.bursts.spawn(BurstMode.FIRE, target.x, Y_WATER + 0.75, target.z, {
          radius: 0.55, endRadius: 4.6, life: 0.95, squash: 0.78, intensity: 1.25,
          fresnel: 1.1, displace: 0.5, opacity: 0.95,
          colorA: 0xffe6a0, colorB: 0xff6a18, colorC: 0x661408,
        });
        fx.decals.spawn(DecalType.SHOCKWAVE, target.x, target.z, {
          radius: 6.5, life: 0.8, width: 0.07, intensity: 1.05,
          colorA: 0xffe0a0, colorB: 0xff6a18,
        });
        fx.decals.spawn(DecalType.SCORCH, target.x, target.z, {
          radius: 5.2, life: 2.6, intensity: 1.1,
          colorA: 0x1a100c, colorB: 0xff6a22,
        });
        fx.sparks.emit(40, {
          x: target.x, y: Y_WATER + 0.55, z: target.z, radius: 0.9,
          speed: 7.5, spread: 1, life: 0.85, size: 9, color: 0xff8a28, gravity: 0.55, vy: 1,
        });
        fx.sparks.emit(28, {
          x: target.x, y: Y_WATER + 0.6, z: target.z, radius: 0.7,
          speed: 9.0, spread: 1, life: 0.45, size: 5.5, color: 0xffe090, gravity: 0.25, vy: 1,
        });
        fx.sparks.emit(18, {
          x: target.x, y: Y_WATER + 0.45, z: target.z, radius: 1.1,
          speed: 2.4, spread: 0.85, life: 1.15, size: 18, color: 0xff7030, gravity: 0.2, vy: 1,
        });
      }
      const b = (age - fly) / boom;
      ring.scale.setScalar(1 + b * 2.2);
      ring.material.uniforms.uTime.value = age;
      ring.material.uniforms.uRadius.value = 8 * (1 + b * 0.4);
...
export function createSkillVfx({ scene }) {
  ...
  function cast(kind, origin, dir, range) {
    ...
    let inst;
    if (kind === 'thunder') inst = spawnThunder(root, o, d, dist, fx);
    else if (kind === 'meteor') inst = spawnMeteor(root, o, d, dist, fx);
    else inst = spawnIce(root, o, d, dist, fx);
    active.push(inst);
    return inst;
  }
...
export const SKILL_CARDS = [
  ...
  { id: 'meteor', name: '陨石', cd: 3.5, range: 22, radius: 8 },
];
